...
function dateToTimestamp(date) {
  ...
  const b = new Date('01 Jan 1970 00:00:00 UTC');
  return a - b;
}
...
function isDateInPeriod(date, period) {
  ...
  const startMilliseconds = Date.parse(period.start);
  const endMilliseconds = Date.parse(period.end);
  let result = false;
  if (
    dateMilliseconds >= startMilliseconds &&
    dateMilliseconds <= endMilliseconds
  ) {
    result = true;
  }
  return result;
...
function formatDate(date) {
  ...
  const month = newData.getUTCMonth() + 1;
  const day = newData.getUTCDate();
  const year = newData.getUTCFullYear();
  let hour = newData.getUTCHours();
  const minute = newData.getUTCMinutes();
  const seconds = newData.getUTCSeconds();
  let a = 'AM';
  if (hour >= 12) {
    a = 'PM';
  }
  ...
  return `${month}/${day}/${year}, ${hour}:${minute.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${a}`;
}
...
function getQuarter(date) {
  const newData = new Date(date);
  const month = newData.getUTCMonth();
  let quarter = 4;
  if (month >= 0 && month <= 2) {
    quarter = 1;
  }
